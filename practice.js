...
function divider(arr) {
  let newArr = [];
  newArr.push(arr.filter(elem => elem % 2 === 0));
  newArr.push(arr.filter(elem => elem % 2 !== 0));
  return newArr;
}
...
function addTen(arr) {
  return arr.map(elem => Number(elem) + 10);
}
...
function makeObj(name,email,password,username) {
  return {
    name:name,
    email:email,
    password:password,
...
users.push(user1);
users.push(makeObj("Cool Name", "cool@email.com", "12345", "CoolOne"));
users.push(makeObj("Unoriginal", "unorignal@emial.com", "ABC", "UNO"));
